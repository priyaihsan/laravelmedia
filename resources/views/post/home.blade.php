<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl lg:p-5 lg:pb-8 lg:gap-5 lg:columns-4 lg:space-y-5 mx-auto p-3 pb-3 gap-3 columns-1 space-y-3">
            @foreach ($posts as $post)
                {{-- start card --}}
                <div class="line-clamp-2">
                    {{-- start modal detail post --}}
                    <button data-modal-target="defaultModal-{{ $post->id }}"
                        data-modal-toggle="defaultModal-{{ $post->id }}" type="button" class="w-full">
                        <img class="rounded-lg" src="{{ asset('storage/' . $post->content_url) }}" alt="" />
                    </button>
                    ...
                    <div class="flex items-center text-left gap-2 w-full ">
                        <img class="rounded-full w-6 h-6" src="{{ $post->user->profile_picture }}" alt="">
                        <h5 class=" text-sm font-normal text-gray-900 dark:text-white">
                            {{ $post->user->name }}</h5>
                    </div>
                </div>
                <!-- Main modal -->
                    ...
                    <div class="relative w-full max-w-2xl max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div class="grid grid-cols-1  lg:grid-cols-2">
                                <img class="lg:rounded-l-lg w-full h-full"
                                        src="{{ asset('storage/' . $post->content_url) }}" alt="">
                                <div class="p-3 flex flex-col justify-between">
                                    <div>
                                        <div class="hidden lg:flex items-end">
                                            <button type="button" data-modal-hide="defaultModal-{{ $post->id }}"
                                        ...
                                        <div>
                                            ...
                                            <div class="flex flex-wrap lg:flex-nowrap items-center gap-2 mx-3 my-3">
                                                @if ($post->user->id != auth()->user()->id)
